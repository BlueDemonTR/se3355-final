...
async function startLobby(lobby, user, io) {
  ...
  const cube = await Cube.findByIdAndUpdate(lobby.cube, { $inc: { playCount: 1 } })

  const cubeContent = []

  const cardData = await getCards(cube.cards)
  ...
  for (const { id, count } of cube.cards) {
  ...
  for (let index = 0; index < attendants.length; index++) {
    ...
    io.to(attendant.account._id.toString()).emit('SET_LOBBY_CARD_DATA', cardData)
  }
